...
//                 <div className="border   rounded-full shadow bg-white hidden md:flex"
...
const Username = ({ name }) => {
    ...
    return (
        <div>
            {/* Mobile avatar */}
            <div
                className="md:hidden border-2 border-gray-200 w-10 h-10 bg-blue-50 hover:bg-blue-100 transition rounded-full flex justify-center items-center shadow active:scale-110 cursor-pointer"
                onClick={() => setToggle(!toggle)}
                role="button"
                aria-label="Toggle menu"
            >
                <p className="font-semibold text-gray-700 hover:text-black">
            ...
            {/* Desktop username */}
            <div
                className="border rounded-2xl shadow bg-white hidden md:flex group cursor-pointer"
                onClick={() => setToggle(!toggle)}
                role="button"
                aria-label="Toggle menu"
            >
                <div className="flex items-center justify-between py-2 px-3 w-full">
                    <p className="text-gray-600 hover:text-black font-semibold text-[18px]">
                        {name}
                    </p>
                    <svg
                        className={`w-5 h-5 text-gray-400 transition-all duration-200 hover:text-black group-hover:text-blue-600 group-hover:translate-x-0.5 ${toggle ? "rotate-90" : ""}`}
                        fill="none"
                        strokeWidth="2"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
